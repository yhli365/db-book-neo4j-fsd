...
public class Friends {
	...
	public void printFriends() {
		System.out.println(printFriends("breadthFirst"));
		System.out.println(printFriends("depthFirst"));
	}

	public String printFriends(String mode) {
		try (Transaction tx = graphDb.beginTx()) {
			Node neoNode = getEndNode(homeNodeId);
			int numberOfFriends = 0;
			String output = "Exec mode: " + mode + "----------\n";
			output += neoNode.getProperty("name") + "'s friends:\n";
			Traverser friendsTraverser;
			if ("breadthFirst".equals(mode)) {
				friendsTraverser = getFriends(neoNode);
			} else {
				friendsTraverser = getFriendsDepth(neoNode);
			}
	...
	public void printWorkers() {
		System.out.println(printWorkers("depthFirst"));
		System.out.println(printWorkers("breadthFirst"));
	}

	public String printWorkers(String mode) {
		String output = "Exec mode: " + mode + "----------\n";
		try (Transaction tx = graphDb.beginTx()) {
			output += "co-worker:\n";
			int number = 0;
			Traverser traverser;
			if ("breadthFirst".equals(mode)) {
				traverser = findWorkerBreadth(getEndNode(homeNodeId));
			} else {
				traverser = findWorker(getEndNode(homeNodeId));
			}
	...
	private Traverser findWorkerBreadth(final Node startNode) {
		TraversalDescription td = graphDb.traversalDescription()//
				.breadthFirst()//
				.relationships(RelTypes.CO_WORKER, Direction.OUTGOING)//
				.relationships(RelTypes.KNOWS, Direction.OUTGOING)//
